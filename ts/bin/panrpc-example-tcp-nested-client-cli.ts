...
class TimeRemote {
  ...
  async GetSystemTime(ctx: IRemoteContext): Promise<number> {
    return 0;
  }
}

class Remote {
  constructor(public Time: TimeRemote) {}
    ...
    await registry.forRemotes(async (remoteID, remote) => {
      ...
      switch (line) {
        ...
        case "c":
          try {
            // eslint-disable-next-line no-await-in-loop
            const res = await remote.Time.GetSystemTime(undefined);

            console.log(res);
          } catch (e) {
            console.error(`Got error for Time.GetSystemTime func: ${e}`);
          }
